...
import numpy as np
import sounddevice as sd # <--- SONIDO REACTIVADO
...
COLOR_FONDO = "#FFF5F2"         # Rosa muy pálido para el fondo
COLOR_FONDO_GRAFICO = "#F5BABB"  # Rosa pálido para el fondo del gráfico
COLOR_ACENTO = "#568F87"         # Verde/turquesa para la onda y detalles
COLOR_TEXTO = "#064232"         # Verde muy oscuro para el texto

class AppOndas(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Visualizador de Ondas Sonoras")
        ...
        self.freq_var = tk.DoubleVar(value=261.63) # Do central (C4)
        ...
        ttk.Scale(marco_controles, from_=100, to=1000, orient="horizontal", variable=self.freq_var).grid(row=0, column=1, sticky="ew", padx=5)

        ttk.Label(marco_controles, text="Intensidad (Volumen)", style="Custom.TLabel").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ttk.Scale(marco_controles, from_=0, to=1.0, orient="horizontal", variable=self.amp_var).grid(row=1, column=1, sticky="ew", padx=5)
    ...
    def reproducir_sonido(self):
        ...
        # Para el audio SIEMPRE usamos muchos puntos (alta tasa de muestreo)
        sample_rate = 44100
